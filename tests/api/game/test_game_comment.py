...
def test_get_game_comment_not_found_returns_410(comment_api):
    with step("Request non-existing game comment by id"), pytest.raises(requests.HTTPError) as exc_info:
        comment_api.get_game_comment(comment_id=NON_EXISTENT_ID)
...
def test_delete_game_comment_not_found_returns_410(comment_api):
    with step("Delete non-existing game comment by id"), pytest.raises(requests.HTTPError) as exc_info:
        comment_api.delete_game_comment(comment_id=NON_EXISTENT_ID)
    with step("Verify status code is 410"):
        assert exc_info.value.response.status_code == 410
...
def test_get_game_comments_for_non_existing_game_returns_410(comment_api):
    with step("Request comments for non-existing game"), pytest.raises(requests.HTTPError) as exc_info:
        comment_api.get_game_comments(game_id=NON_EXISTENT_ID)
    with step("Verify status code is 410"):
        assert exc_info.value.response.status_code == 410
...
def test_read_game_comments_for_non_existing_game_returns_410(comment_api):
    with step("Mark comments as read for non-existing game"), pytest.raises(requests.HTTPError) as exc_info:
        comment_api.read_game_comments(game_id=NON_EXISTENT_ID)
    with step("Verify status code is 410"):
        assert exc_info.value.response.status_code == 410
...
def test_like_game_comment_not_found_returns_410(comment_api):
    with step("Like non-existing game comment"), pytest.raises(requests.HTTPError) as exc_info:
        comment_api.post_game_comment_like(comment_id=NON_EXISTENT_ID)
    with step("Verify status code is 410"):
        assert exc_info.value.response.status_code == 410
...
def test_get_game_comment_without_token_returns_401_or_403(configs):
    ...
    with step("Get game comment without auth token"), pytest.raises(requests.HTTPError) as exc_info:
        api.get_game_comment(comment_id=NON_EXISTENT_ID)
...
def test_delete_game_comment_without_token_returns_401_or_403(configs):
    ...
    with step("Delete game comment without auth token"), pytest.raises(requests.HTTPError) as exc_info:
        api.delete_game_comment(comment_id=NON_EXISTENT_ID)
    with step("Verify status code is 401 or 403"):
        assert exc_info.value.response.status_code in (401, 403)


@pytest.mark.regression
def test_post_game_comment_for_non_existing_game_returns_documented_error(comment_api):
    payload = {"content": "autotest game comment"}
    with step("Create comment for non-existing game"), pytest.raises(requests.HTTPError) as exc_info:
        comment_api.post_game_comment(game_id=NON_EXISTENT_ID, payload=payload)
    with step("Verify status code is documented for POST /v1/games/{id}/comments"):
        assert exc_info.value.response.status_code in (400, 401, 403, 410)


@pytest.mark.regression
def test_update_game_comment_for_non_existing_id_returns_documented_error(comment_api):
    payload = {"content": "updated by autotest"}
    with step("Update non-existing game comment"), pytest.raises(requests.HTTPError) as exc_info:
        comment_api.update_game_comment(comment_id=NON_EXISTENT_ID, payload=payload)
    with step("Verify status code is documented for PATCH /v1/games/comments/{id}"):
        assert exc_info.value.response.status_code in (400, 401, 403, 410)


@pytest.mark.regression
def test_delete_game_comment_like_for_non_existing_comment_returns_documented_error(comment_api):
    with step("Delete like for non-existing game comment"), pytest.raises(requests.HTTPError) as exc_info:
        comment_api.delete_game_comment_like(comment_id=NON_EXISTENT_ID)
    with step("Verify status code is documented for DELETE /v1/games/comments/{id}/likes"):
        assert exc_info.value.response.status_code in (401, 403, 409, 410)
