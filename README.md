...
## Overview
API autotests for DM services based on Swagger contracts:
- `swagger_account.json`
- `swagger_common.json`
- `swagger_community.json`
- `swagger_forum.json`
- `swagger_game.json`
- `swagger_messaging.json`
...
- `game`: covered baseline for lists and key entities (`game`, `attribute schema`, `room`, `post`, `comment`, `character`), but still partial vs full swagger scope.
- `messaging`: chat/dialogues/messages read scenarios, not-found checks, auth-negative cases, and invalid payload checks for chat post.
